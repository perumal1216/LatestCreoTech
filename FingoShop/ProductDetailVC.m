...
//    [_productsImageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"place"]];
   ...
   // NSArray *keysArr = [additionalDict allKeys];
    switch (keysArr.count) {
        ...
        case 1:
        // NSDictionary *additionalDict = [keysArr objectAtIndex:0];
...
-(void)callAddToCartService:(NSString*)qty
{
  ...
  NSDictionary *dict  = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"customer_id"]],@"customerid",[NSString stringWithFormat:@"%@",[productDetailsDict objectForKey:@"id"]],@"product",@"1",@"qty", nil];
    [serviceconn AddToCart:dict];
}
...
- (void)jsonData:(NSDictionary *)jsonDict
{
    ...
    {
        if ([[jsonDict objectForKey:@"code"] isEqualToString:@"1"]) {
...
- (IBAction)btnLikeClicked:(UIButton *)sender {
        ...
        else
        {
          ...
          //  [self callAddToWishListService:[productDetailsDict objectForKey:@"entity_id"]];
            NSDictionary *post_dict = @{@"product":[productDetailsDict objectForKey:@"id"],@"customer_id":[[NSUserDefaults standardUserDefaults] objectForKey:@"customer_id"],@"SID":[[NSUserDefaults standardUserDefaults] objectForKey:@"sessionid"]};
