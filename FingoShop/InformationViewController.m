...
-(void)callGetShippingService:(NSString *)pincode
{
    [APPDELEGATE showCustomLoader:self];
    serviceconn = [[ServiceConnection alloc]init];
    serviceconn.delegate = self;
    ServiceType=@"CityAndState";
    [serviceconn getCityAndState:pincode];
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField == self.txtfldFirstName ||textField == self.txtfldLastName || textField == self.txtfldCity) {
    ...
    else if (textField == self.txtfldZipCode)
     {
         ...
         if (newLength == 7) {
             [_txtfldZipCode resignFirstResponder];
             [self callGetShippingService:_txtfldZipCode.text];
         }
         if(newLength >= 7){
             return NO;
         }
...
//
    ...
    }else if ([ServiceType isEqualToString:@"CityAndState"])
    {
        _txtfldCity.text = [jsonDict objectForKey:@"city"];
        _txtfldState.text = [jsonDict objectForKey:@"state"];
    }else{
