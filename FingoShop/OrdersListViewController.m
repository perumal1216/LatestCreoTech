...
//    cell.btnCancelOrder.tag = indexPath.section;
    ...
    NSArray *ProductArr = [ordersArray objectAtIndex:0] ;
   // NSLog(@"product array is :%@",ProductArr);
    
    // NSDictionary *dict = [ordersArray objectAtIndex:indexPath.section];
     NSDictionary *dict = [ProductArr objectAtIndex:0];
    NSDictionary *additionalDataDict = [dict objectForKey:@"additional_data"];
    //[[ordersArray objectAtIndex:indexPath.section] objectForKey:@"additional_data"];
    
    cell.lblOrderNo.text=[NSString stringWithFormat:@"Order No : %@",[additionalDataDict objectForKey:@"increment_id"]];
    cell.lblTotalItems.text=[additionalDataDict objectForKey:@"total_item_count"];
    //[NSString stringWithFormat:@"Total Items(%lu)",(unsigned long)ProductArr.count];
    cell.lblDeliveryEstimate.text=[dict objectForKey:@"updated_at"];
    cell.lblStatus.text=[additionalDataDict objectForKey:@"status"];
    cell.lblQty.text=[dict objectForKey:@"total_qty_ordered"];
